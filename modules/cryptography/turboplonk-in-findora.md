...
* $$S_{\sigma1}(X)Sσ1​(X)$$'s evaluation on $$1,\omega, ..., \omega^{n-1}$$ equals $$\sigma^*(x)$$'s evaluation on $$1, 2, ..., n$$.
* $$S_{\sigma2}(X)Sσ2​(X)$$'s evaluation on $$1, \omega, ..., \omega^{n-1}$$ equals $$\sigma^*(x)$$'s evaluation on $$n+1, n+2, ..., 2n$$.
...
**Step 1: commit all polynomials**[**​**](https://wiki.findora.org/docs/modules/cryptography/turbo\_plonk#step-1-commit-all-polynomials)
...
**Step 2: precompute the two helper polynomials**[**​**](https://wiki.findora.org/docs/modules/cryptography/turbo\_plonk#step-2-precompute-the-two-helper-polynomials)
...
and store its coset FFT representation. This is done by first observing that $$L_1(X)$$ evaluates to $$nn$$ on $$X = 1$$ and $$00$$ otherwise in $$H$$. We can perform an inverse FFT to convert it back to the coefficient representation (which indeed looks nontrivial). Then, we perform a coset FFT, which gives us the prepared version of this polynomial.
...
**Step 1: assemble public inputs**[**​**](https://wiki.findora.org/docs/modules/cryptography/turbo\_plonk#step-1-assemble-public-inputs)
...
**Step 7: commit the split quotient polynomials, without hiding**[**​**](https://wiki.findora.org/docs/modules/cryptography/turbo\_plonk#step-7-commit-the-split-quotient-polynomials-without-hiding)
...
**Step 8: open the polynomials at a random point**[**​**](https://wiki.findora.org/docs/modules/cryptography/turbo\_plonk#step-8-open-the-polynomials-at-a-random-point)
...
**Step 9: compute the linearization polynomial**[**​**](https://wiki.findora.org/docs/modules/cryptography/turbo\_plonk#step-9-compute-the-linearization-polynomial)
...
**Step 11: output the full proof**[**​**](https://wiki.findora.org/docs/modules/cryptography/turbo\_plonk#step-11-output-the-full-proof)
...
### Verifier[​](https://wiki.findora.org/docs/modules/cryptography/turbo\_plonk#verifier) <a href="#verifier" id="verifier"></a>

The verifier reads the full proof above and proceed as follows.

**Step 1: compute all the challenges**[**​**](https://wiki.findora.org/docs/modules/cryptography/turbo\_plonk#step-1-compute-all-the-challenges)
...
**Step 2: compute** $$r(\zeta)$$[**​**](https://wiki.findora.org/docs/modules/cryptography/turbo\_plonk#step-2-compute-rzeta)
...
**Step 3: assemble** $$\mathsf{cm}_r$$[**​**](https://wiki.findora.org/docs/modules/cryptography/turbo\_plonk#step-3-assemble-mathsfcm\_r)
